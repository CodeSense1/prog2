...
bool writeToFile(const std::string& filename, std::map< std::string, Student* >& data) {

    std::ofstream file(filename);

    if (!file) {
        std::cout << "MITÄ VITTUA!" << std::endl;
        return false;
    }
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            Student* student = student_numbers.at(id);
            ...
            if (!writeToFile(file_name, user_ids))
                return EXIT_FAILURE;
