...
public:
    ...
    // Return wether book is loaned
    bool isBookLoaned(const std::string& book_title);

    // Return book by its name
    Loan* findLoanByBookTitle(const std::string& bookTitle);
    bool isAccountRegistered(const std::string& accountName);
    bool isBookValid(const std::string& bookTitle);
