...
void combine(std::vector<NumberTile> &arr)
{
    ...
    {
        if (arr.at(i).getValue() == arr.at(i + 1).getValue() && arr.at(i).getValue() != 0)
        {
...
int main()
{
    ...
    try {
        OPTIONAL_GOAL = (input != "") ? std::stoi(input) : DEFAULT_GOAL;
    } catch (std::invalid_argument) {
        OPTIONAL_GOAL = DEFAULT_GOAL;
    }
